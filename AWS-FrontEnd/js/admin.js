...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // The loadAdminData function will be called from admin.html after authentication
  ...
  async function fetchProducts() {
    try {
      const response = await authenticatedFetch(`${API_BASE_URL}/api/products`);
  ...
  productTableBody.addEventListener('click', (e) => {
    if (e.target.classList.contains('show-id-btn')) {
      ...
      console.log('Selected productId for deletion:', selectedProductId);
  ...
  confirmDelete.addEventListener('click', async () => {
    ...
    try {
      console.log(`Attempting DELETE for productId: ${selectedProductId}`);
      ...
      if (response.status === 200 && data.statusCode === 200) {
        console.log(`Success: Product ${selectedProductId} deleted`, data.body.deletedProduct);
        deleteModal.style.display = 'none';
        selectedProductId = null;
        await fetchProducts();
      } else {
