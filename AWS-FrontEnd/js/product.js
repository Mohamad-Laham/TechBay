...
async function authenticatedFetch(url, options = {}) {
  ...
  return fetch(url, {
    ...options,
    headers: {
      ...(idToken ? { 'Authorization': `Bearer ${idToken}` } : {}), // Ensure Bearer prefix
    ...
    try {
      ...
      if (isAuthenticated) {
        ...
        addToCartBtn.addEventListener('click', async () => {
          ...
          const innerData = { productId, quantity, category: foundCategory };
          ...
          console.log('idToken:', idToken); // Debug token presence
          console.log('Sending Authorization:', idToken ? `Bearer ${idToken}` : 'No token');

          try {
            const response = await authenticatedFetch('https://7q4o04d3nh.execute-api.us-east-1.amazonaws.com/prod/api/cart', {
              method: 'POST',
              body: JSON.stringify(requestData) // Send the wrapped structure
            });
            ...
            if (response.status === 401) {
              ...
              login(); // Ensure login updates idToken
              return;
            }
